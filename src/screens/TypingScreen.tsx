...
const TypingScreen: React.FC = () => {
  ...
  const [isDoctorTurn, setIsDoctorTurn] = useState(false);
  const flatListRef = useRef<FlatList>(null);

  const handleSend = () => {
    if (!inputText.trim() || isDoctorTurn) return;
    ...
    setInputText('');
    setIsDoctorTurn(true); // Now it's doctor's turn to respond
  };

  const startRecording = async () => {
    if (!isDoctorTurn) return;
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      if (response.data.transcript) {
        // Add doctor's message without generating any response
        addMessage({
          text: response.data.transcript,
          sender: 'doctor',
          isOption: false
        }, false); // false = don't speak this message
        
        // Switch back to patient's turn
        setIsDoctorTurn(false);
      } else {
      ...
      // Still switch back to patient's turn even if failed
      setIsDoctorTurn(false);
    } finally {
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput
              style={[
                ...
                isDoctorTurn && styles.inputDisabled
              ]}
              value={inputText}
              ...
              blurOnSubmit={false}
              editable={!isRecording && !isDoctorTurn}
            />
